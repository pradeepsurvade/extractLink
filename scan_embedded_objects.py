...
def parse_docx(docx_path):
    ...
    def table_context(ole):
        ...
        # ── Row label: always taken from column 0 of the current row ─────────
        # If col 0 is empty or IS the OLE column → row label is blank.
        row_label_text = ''
        if tcs and ole_col != 0:
            row_label_text = text(tcs[0])[:100]

        # ── Column headers: shown only when the first row is a true header ────
        # A header row: no OLE objects, all cells ≤ 100 chars, at least one cell
        # non-empty, and the OLE is NOT in the first row.
        row_label_col_hdr = ''
        url_col_hdr       = ''

        if all_rows and tr is not all_rows[0]:
            first_row     = all_rows[0]
            first_has_ole = any(
                e.tag.split('}')[-1] == 'OLEObject' for e in first_row.iter()
            )
            first_row_tcs  = first_row.findall(f'{{{NS_W}}}tc')
            all_cells_short = all(len(text(tc)) <= 100 for tc in first_row_tcs)
            has_any_text    = any(text(tc) for tc in first_row_tcs)

            if not first_has_ole and all_cells_short and has_any_text:
                # Col 0 header = Row Label Column Header (even when col 0 data is empty)
                row_label_col_hdr = text(first_row_tcs[0]) if first_row_tcs else ''
                # OLE column header = URL Column Header
                url_col_hdr = (text(first_row_tcs[ole_col])
                               if ole_col is not None and ole_col < len(first_row_tcs)
                               else '')
