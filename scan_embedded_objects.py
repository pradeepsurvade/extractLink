...
def _clean_str(v):
    """Strip control characters that openpyxl rejects (all chars below 0x20 except tab)."""
    if not v: return ''
    return re.sub(r'[\x00-\x08\x0b-\x1f\x7f]', '', str(v))
...
def parse_docx(docx_path):
    try:
        ...
        raise ValueError("File is not a valid .docx (not a ZIP file)")
    ...
    def table_context(ole):
        ...
        # ── Row label: always from column 0 of the OLE's row ─────────────────
        # If col 0 is empty or IS the OLE column → row label is blank.
        row_label_text = ''
        if tcs and ole_col != 0:
            row_label_text = text(tcs[0])[:100]

        # ── Header detection: first row is a header when any of its cells ─────
        # have a non-white / non-empty background fill OR bold text.
        # White variants: 'FFFFFF', 'auto', '' (no fill).
        # This reliably distinguishes styled header rows from plain data rows.
        def _is_header_row(row):
            """True if the row has coloured shading or bold text — marks it as a header."""
            if any(e.tag.split('}')[-1] == 'OLEObject' for e in row.iter()):
                return False   # a row with OLE objects is always a data row
            WHITE = {'ffffff', 'FFFFFF', 'auto', ''}
            for tc in row.findall(f'{{{NS_W}}}tc'):
                tcPr = tc.find(f'{{{NS_W}}}tcPr')
                if tcPr is not None:
                    shd = tcPr.find(f'{{{NS_W}}}shd')
                    if shd is not None:
                        fill = shd.get(f'{{{NS_W}}}fill', '')
                        if fill not in WHITE:
                            return True   # non-white cell background
                # Any bold run in the cell also qualifies
                if any(r.find(f'.//{{{NS_W}}}b') is not None
                       for r in tc.iter(f'{{{NS_W}}}r')):
                    return True
            return False

        # ── Column headers: only when first row is a header AND OLE row ≠ first row
        row_label_col_hdr = ''
        url_col_hdr       = ''

        if all_rows and tr is not all_rows[0] and _is_header_row(all_rows[0]):
            hdr_tcs = all_rows[0].findall(f'{{{NS_W}}}tc')
            # Row Label Column Header = header of col 0
            row_label_col_hdr = text(hdr_tcs[0]) if hdr_tcs else ''
            # URL Column Header = header of the OLE's column
    ...
    for ole in tree.iter():
        ...
        if obj_type == 'BIN': continue                 # skip unrecognised binary objects
...
def _zip_ext(raw):
    """Peek inside ZIP bytes to return .xlsx/.docx/.pptx/.zip, or '' if not a valid ZIP."""
    try:
        ...
        return ''   # not a valid ZIP — caller will skip or treat as raw bytes
...
def _doc_build_heading_map(wd_stream):
    """
    Scan the entire WordDocument stream once and return a sorted list of
    (byte_pos, section_label) for every valid top-level section heading.

    Each entry's byte_pos is the position of the paragraph-end \\r byte, so
    'the heading before OLE at X' is simply the last entry with byte_pos < X.

    Validity rules (filters out sub-headings, footer text, TOC artefacts):
      - ALL-CAPS text with ≥ 4 alpha characters, no \\x07 cell separators
      - No underscores (eliminates SOP file-name strings like SOP_FIN_ACCT_…)
      - Single-word entries must be a known section keyword (OBJECTIVE, APPENDIX, …)
        OR have > 8 alpha characters — short codes like ASKB / PAGE are rejected
    """
    SENT = '\ue001'
    ALLOWED_SINGLE = {'OBJECTIVE', 'DEFINITIONS', 'POLICIES', 'APPENDIX', 'OVERVIEW'}

    def _valid(txt):
        t = re.sub(r'^\d+', '', txt).strip()   # strip leading TOC numbers
        t = re.sub(r'\d+$', '', t).strip()      # strip trailing page numbers
        if not t or '_' in t:
            return False
        ...
        if len(words) == 1:
            return words[0] in ALLOWED_SINGLE or len(words[0]) > 8
        return True

    raw_results = []          # (byte_pos, label)
    i = 0; para_start = 0; n = len(wd_stream)
    while i < n:
        if wd_stream[i] == 0x0d:
            para_bytes = wd_stream[para_start:i]
            para_start = i + 1
            if len(para_bytes) >= 4:
                try:
                    t = para_bytes.decode('cp1252', errors='replace')
                    t = t.replace('\x14\x01\x15', SENT)
                    t = re.sub(r'\x13[^\x14\x15]*\x14', '', t)
                    t = re.sub(r'[\x00-\x1f]', '', t).replace(SENT, '').strip()
                    if t and '\x07' not in t and _valid(t):
                        # Clean off TOC digit artefacts, recover number
                        m_num = re.match(r'^(\d+)(.*)', t)
                        num   = m_num.group(1) if m_num else ''
                        clean = re.sub(r'^\d+', '', t).strip()
                        clean = re.sub(r'\d+$', '', clean).strip()
                        label = f"{num}. {clean}" if num else clean
                        raw_results.append((i, label))
                except Exception:
                    pass
        i += 1

    # Keep only the LAST (body) occurrence of each label — de-duplicate on clean label
    seen = {}
    for pos, label in raw_results:
        seen[label] = pos          # later byte position overwrites earlier (TOC)
    return sorted((pos, lbl) for lbl, pos in seen.items())


def _doc_table_context(wd_stream, ole_byte_pos, heading_map):
    """
    Given the WordDocument stream, the byte position of an OLE placeholder, and
    a pre-built heading_map [(byte_pos, label) ...] sorted ascending, return:
        (section, row_label, rl_col_hdr, url_col_hdr)
    """
    SENT = '\ue001'
    ...
    # Section = last heading whose byte_pos is strictly before the OLE position
    section_name = 'DOCUMENT'
    for pos, label in reversed(heading_map):
        if pos < ole_byte_pos:
            section_name = label
            break
    ...
    # Table context: decode window around OLE
    win_txt = _clean(wd_stream[max(0, ole_byte_pos - 2000):ole_byte_pos + 200])
    paras   = win_txt.split('\r')

    # Find the para containing the OLE marker
    ole_para_idx = None
    ...
    # ── Locate the OLE cell ─────────────────────────────────────────────────
    ole_cell_idx = next((i for i, c in enumerate(all_cells) if '\x01' in c), None)

    if ole_cell_idx is not None:
        # ── Detect multi-row flat table (header row + data rows) ─────────────
        # n_cols = consecutive non-empty leading cells = header width
        n_cols = 0
        for c in all_cells:
            if not c: break
            n_cols += 1
        ...
        row_width  = n_cols + 1           # content cells + 1 row-end empty cell
        hdr_cells  = all_cells[:n_cols]   # first row = column headers
        data_cells = all_cells[row_width:]

        # Find the OLE inside DATA rows (skipping the header row)
        ole_data_idx = next((i for i, c in enumerate(data_cells) if '\x01' in c), None)

        if ole_data_idx is not None:
            # Multi-row table: pick label from the same data row as the OLE
            col_in_row = ole_data_idx % row_width
            row_start  = (ole_data_idx // row_width) * row_width
            data_row   = data_cells[row_start:row_start + row_width]
            url_col_hdr = hdr_cells[col_in_row] if col_in_row < len(hdr_cells) else ''
            for ci, c in enumerate(data_row[:n_cols]):
                v = c.replace('\x01', '').strip()
                if v and ci != col_in_row:
                    row_label  = v[:100]
                    rl_col_hdr = hdr_cells[ci] if ci < len(hdr_cells) else ''
                    break
        else:
            # OLE is in the first (header) row — sparse/inline single row
            # Skip activity-code cells like "12.A.7.ABZP/FA accountant"
            for ci, c in enumerate(all_cells):
                if ci == ole_cell_idx: continue
                v = c.replace('\x01', '').strip()
                if v and not re.match(r'^\d+\.\w', v):
                    row_label = v[:100]
                    break

        # ── Column headers from a preceding table-header paragraph ────────────
        if not url_col_hdr:
            for p in reversed(paras[:ole_para_idx]):
                if '\x07' not in p or '\x01' in p: continue
                hc = [c.strip() for c in p.split('\x07')]
                if sum(1 for c in hc if c) >= 2:
                    if ole_cell_idx < len(hc):
                        url_col_hdr = hc[ole_cell_idx]
                    for ci, c in enumerate(hc):
                        if ci == ole_cell_idx: continue
                        if c.strip():
                            rl_col_hdr = c.strip()
                            break
    ...
    # ── Fallback: nearest preceding non-table paragraph as row label ─────────
    # Prefer process-step labels (starting with a digit) over plain prose.
    if not row_label:
        best_step  = ''
        best_prose = ''
        for p in reversed(paras[:ole_para_idx]):
            t = re.sub(r'[\x00-\x08\x0b-\x1f]', '', p).strip()
            if not t or '\x07' in t or '\x01' in t: continue
            alpha = [c for c in t if c.isalpha()]
            if len(alpha) < 4 or all(c.isupper() for c in alpha): continue
            if not best_step and re.match(r'\d', t):
                best_step = t[:100]
            if not best_prose:
                best_prose = t[:100]
            if best_step:
                break
        row_label = best_step or best_prose
...
def parse_doc(doc_path):
    ...
    # Build document-wide heading map once (section lookup for every OLE)
    _heading_map = _doc_build_heading_map(wd_stream)
    ...
    for storage in storages:
        ...
        if 'Package' in streams:
            ...
            if raw[:2] == b'PK':                        # direct ZIP (xlsx, docx, …)
                ...
                if not file_ext: continue               # PK magic but not a valid ZIP — skip
                file_bytes = raw
            else:                                        # Ole10Native wrapper
                lbl, _src, fb = _parse_ole10native(b'\x04\x00\x00\x00\x02\x00' + raw)
                file_bytes = fb if fb else raw
                file_label = os.path.basename(_src) if _src else lbl
                file_ext   = os.path.splitext(file_label)[1].lower()

        elif '\x01Ole10Native' in streams:
            raw = _r('\x01Ole10Native')
            lbl, _src, fb = _parse_ole10native(raw)
            file_bytes = fb
            file_label = lbl or os.path.basename(_src)
            file_ext   = os.path.splitext(file_label)[1].lower()
        ...
        # Skip bare BMP icon images and Windows Paint (PBrush) objects — not real docs
        if file_bytes[:2] == b'BM' and file_ext not in ('.bmp',): continue
        if len(file_bytes) < 64: continue
        if 'PBrush' in prog_id: continue
        ...
        if ole_positions:
            ...
            section, row_label, rl_col_hdr, url_col_hdr = _doc_table_context(wd_stream, ctx_pos, _heading_map)
        else:
...
def extract_files(objects, doc_path):
    ...
    if is_docx:
        try:
            ...
            print(f"  [!] Cannot open .docx as ZIP: {e}")
            for obj in objects:
                obj['saved_path'] = ''; obj['verify_status'] = f'✘ not a zip file'
            return
...
def write_report(objects):
    ...
    for i,obj in enumerate(objects):
        ...
        cell(r,1,_clean_str(obj.get('folder','')))
        cell(r,2,_clean_str(obj.get('source_doc','')))
        cell(r,3,_clean_str(obj['section']))
        cell(r,4,_clean_str(obj['row_label']),          wrap=True)
        cell(r,5,_clean_str(obj.get('row_label_col_hdr','')))
        cell(r,6,_clean_str(obj['object_type']))
        cell(r,7,_clean_str(url),                       color="0563C1",wrap=True)
        cell(r,8,_clean_str(obj.get('url_col_hdr','')))
...
def main():
    ...
    for i, path in enumerate(all_files, 1):
        ...
        except (zipfile.BadZipFile, ValueError) as ex:
            print(f"  [!] Skipped — {ex}"); continue
        except Exception as ex:
            print(f"  [!] Failed: {type(ex).__name__}: {ex}"); continue
