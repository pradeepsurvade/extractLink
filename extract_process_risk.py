...
def _extract(table) -> tuple:
    ...
    # Use deduplicated cells for header row
    hdr_cells = _unique_cells(rows[start])
    base_hdrs = [
        _text(c).strip() or f"Col{i+1}"
        for i, c in enumerate(hdr_cells)
    ]
    ref_hdr = base_hdrs[0] if base_hdrs else None
    n_cols  = len(base_hdrs)

    logical = []
    for row in rows[start + 1:]:
        # Deduplicate data row cells too
        ...
        ref = rd.get(ref_hdr, "").strip() if ref_hdr else ""
...
def write(blocks: list, path: str, skipped: list | None = None,
    ...
    for blk in blocks:
        ...
        #  Per-file sub-header row (dark blue, shows actual column names) 
    ...
    if skipped:
        row_colors = [
            "FFF2CC" if r[2] == "Table Not Found" else "FFFFFF"
            for r in skipped
        ]
        _err_sheet(
            wb,
