...
#   A = Folder Name
#   B = Word Doc Name
#   C = Header Type   (value of Header Content 1 for every row in the file)
#   D+ = Header Content 1, Header Content 2, ...
CS = 4   # content now starts at column D (1=A 2=B 3=C(HeaderType) 4=D)
...
def write(blocks: list, path: str, skipped: list | None = None,
    ...
    total_cols = CS - 1 + max_cc   # A + B + C(HeaderType) + content cols
    ...
    for blk in blocks:
        ...
        # "Header Type" for this file = the value of Header Content 1 on every row.
        # Header Content 1 is the first column label from the table (e.g. "Reference").
        header_type = blk.col_list[0] if blk.col_list else ""

        ref_col_name = blk.base_hdrs[0] if blk.base_hdrs else ""

        # Sub-header row (column name labels) — dark blue
        _fn(ws.cell(cur, 1), blk.folder)
        _fn(ws.cell(cur, 2), blk.filename)
        _sh(ws.cell(cur, 3), header_type)          # Header Type col (same blue)
        ...
        for lr in blk.rows:
            ...
            # If Header Content 1 value equals the column header name itself
            # (e.g. data cell contains "Reference"), treat as sub-header row
            if ref_val.lower() == ref_col_name.lower():
                _sh(ws.cell(cur, 3), header_type)
                for i, h in enumerate(blk.base_hdrs):
                    _sh(ws.cell(cur, CS + i), lr.get(h, ""))
                for ci in range(CS + n_own, total_cols + 1):
                    _sh(ws.cell(cur, ci), "")
            else:
                # Normal data row — white; Header Type col keeps dark-blue throughout
                _sh(ws.cell(cur, 3), header_type)
                for i, h in enumerate(blk.base_hdrs):
