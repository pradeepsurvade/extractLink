...
TRIGGER_PATTERNS = [
    ...
    # "Transaction ABAVN" — bare Transaction prefix + UPPERCASE-only token
    # Uppercase-only ensures plain English words like "management" are ignored
    re.compile(r'\bTransaction\s+([A-Z][A-Z0-9_\-]{1,29})\b'),
    # "Open Transaction \"ABAA\"" / "Open Transaction 'ABAA'" — code in quotes after Transaction
    # "Open Transaction \"ABAA\"" / "Open Transaction 'ABAA'" / "Open Transaction ABAA"
    # Prefix must be exactly "Open Transaction" — no extra words in between
    # Handles straight quotes (", ') AND Word smart/curly quotes (“”‘’)
    re.compile(
        r'\bOpen\s+Transaction\s*[\u201c\u201d\u2018\u2019"\']([ A-Z][A-Z0-9_\-]{1,29})[\u201c\u201d\u2018\u2019"\']?',
        re.IGNORECASE),
...
# Add any word here (without punctuation) to exclude it from extraction.
# e.g. 'DIFF.' in a sentence becomes 'DIFF' after dot-stripping, which matches 'DIFF' here.
NOT_A_TCODE = {
    ...
    # Custom exclusions — add words here without punctuation (DIFF covers DIFF.)
    'DIFF', 'DEP',
}



def strip_tcode_prefix(code: str) -> str:
    """Remove leading T_CODE_ prefix (case-insensitive) from extracted codes.
    e.g. T_CODE_ALR_87012006 -> ALR_87012006
    """
    prefix = 'T_CODE_'
...
# ---------------------------------------------------------------------------
# STANDALONE PATTERN RULES
# Each entry is a (compiled_regex, description) tuple.
# The regex must have exactly one capture group — the transaction code.
# To disable a rule, comment out its line below.
# ---------------------------------------------------------------------------
STANDALONE_PATTERNS = [
    # Any word containing S_ALR_ is treated as a transaction code
    (re.compile(r'\b(S_ALR_[A-Z0-9_]+)\b', re.IGNORECASE), "S_ALR_ prefix rule"),
]


def extract_tcodes_from_text(text: str) -> list:
    """Return list of (tcode, source_sentence) — trigger-phrase + standalone rules."""
    results = []
    ...
    # ── Trigger-phrase based extraction ──────────────────────────────────────
    for pattern in TRIGGER_PATTERNS:
        for m in pattern.finditer(text):
            ...
            if is_valid_tcode(candidate) and candidate not in seen:
                ...
                candidate = strip_tcode_prefix(candidate)
                results.append((candidate, text_stripped))

    # ── Standalone pattern rules (comment out individual lines to disable) ───
    for pattern, _description in STANDALONE_PATTERNS:
        for m in pattern.finditer(text):
...
def build_sheet5(wb, tcode_data, others, all_docx_paths, styles):
    """
    Sheet 5: Exclusive document count per transaction code.

    Logic:
      - For each T-code: count documents where ONLY that code appears (exclusive docs)
      - 'Multiple': count documents where MORE THAN ONE T-code appears
      - 'No Transaction Code': count documents with zero T-codes (others list)
      - Column 2 total = total number of documents scanned
    """
    ws = wb.create_sheet("T-Code Exclusivity")
    hdr_font, hdr_fill, hdr_align, medium_border, thin_border = styles

    tcode_font   = Font(name='Courier New', bold=True, color='1F4E79', size=10)
    multi_font   = Font(name='Arial', bold=True, color='C07000', size=10)
    none_font    = Font(name='Arial', bold=True, color='C00000', size=10)
    total_font   = Font(name='Arial', bold=True, color='FFFFFF', size=10)
    count_font   = Font(name='Arial', bold=True, size=10)
    top_align    = Alignment(horizontal='left',   vertical='center')
    cnt_align    = Alignment(horizontal='center', vertical='center')
    alt_fill     = PatternFill('solid', start_color='EBF3FB')
    base_fill    = PatternFill('solid', start_color='FFFFFF')
    multi_fill   = PatternFill('solid', start_color='FFF2CC')
    none_fill    = PatternFill('solid', start_color='FFE7E7')
    total_fill   = PatternFill('solid', start_color='1F4E79')

    apply_header(ws, ['Transaction Code', 'Distinct Document Count'],
                 hdr_font, hdr_fill, hdr_align, medium_border)
    ...
    # ── Categorise every document ─────────────────────────────────────────────
    # exclusive_docs[tcode] = set of docs where ONLY this tcode appears
    exclusive_docs = {tcode: set() for tcode in tcode_data}
    multiple_docs  = set()   # docs with > 1 tcode
    no_code_docs   = set(others)  # docs with 0 tcodes

    for doc, tcodes in doc_to_tcodes.items():
        if len(tcodes) > 1:
            multiple_docs.add(doc)
        elif len(tcodes) == 1:
            only_tcode = next(iter(tcodes))
            if only_tcode in exclusive_docs:
                exclusive_docs[only_tcode].add(doc)

    total_docs = len(doc_to_tcodes) + len(no_code_docs)

    # ── Data rows: one per tcode (only those with exclusive docs > 0) ─────────
    # Sort by count descending, then alphabetically
    tcode_rows = [
        (tcode, len(docs))
        for tcode, docs in exclusive_docs.items()
    ]
    tcode_rows.sort(key=lambda x: (-x[1], x[0]))

    row = 2
    for i, (tcode, cnt) in enumerate(tcode_rows):
        fill = alt_fill if (i % 2 == 0) else base_fill
    ...
    # ── Multiple row ──────────────────────────────────────────────────────────
    c = ws.cell(row=row, column=1, value='Multiple')
    c.font = multi_font; c.fill = multi_fill
    c.alignment = top_align; c.border = thin_border

    c = ws.cell(row=row, column=2, value=len(multiple_docs))
    c.font = count_font; c.fill = multi_fill
    c.alignment = cnt_align; c.border = thin_border
    ws.row_dimensions[row].height = 24
    row += 1

    # ── No Transaction Code row ───────────────────────────────────────────────
    c = ws.cell(row=row, column=1, value='No Transaction Code')
    c.font = none_font; c.fill = none_fill
    c.alignment = top_align; c.border = thin_border

    c = ws.cell(row=row, column=2, value=len(no_code_docs))
    c.font = count_font; c.fill = none_fill
    c.alignment = cnt_align; c.border = thin_border
    ws.row_dimensions[row].height = 24
    row += 1

    # ── Total row ─────────────────────────────────────────────────────────────
    c = ws.cell(row=row, column=1, value='Total Documents')
    c.font = total_font; c.fill = total_fill
    c.alignment = top_align; c.border = medium_border

    c = ws.cell(row=row, column=2, value=total_docs)
    c.font = total_font; c.fill = total_fill
    c.alignment = cnt_align; c.border = medium_border
...
def build_excel(tcode_data: dict, others: list, output_path: str):
    ...
    build_sheet5(wb, tcode_data, others, None, shared_styles)
