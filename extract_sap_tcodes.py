...
def second_pass_scan(all_docx: list, folder: str, known_tcodes: set) -> dict:
    ...
    for filepath in all_docx:
        ...
        # Collect all text blocks (paragraphs + table cells)
        ...
        for text in text_blocks:
            ...
            for m in pattern.finditer(text_stripped):
                ...
                new_hits[tcode].append((rel_path, text_stripped))
...
def merge_second_pass(tcode_data: dict, new_hits: dict) -> tuple:
    ...
    Only adds (tcode, rel_path, sentence) combos not already present.
    Returns (updated tcode_data, count of new mappings added).
    """
    added = 0
    for tcode, hits in new_hits.items():
        existing = tcode_data.get(tcode, [])
        # Build a set of (rel_path, sentence) already recorded
