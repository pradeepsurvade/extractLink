...
def scan_folder(folder: str):
    ...
    # ── Pass 2: re-scan all docs for bare occurrences of discovered codes ──────
    known_tcodes = set(tcode_data.keys())
    new_hits = second_pass_scan(all_docx, folder, known_tcodes)
    tcode_data, added = merge_second_pass(tcode_data, new_hits)

    # Promote docs from 'others' if Pass 2 found codes in them
    still_others = []
    for rel_path in others:
        found_in_pass2 = any(
            any(rp == rel_path for rp, _ in entries)
            for entries in tcode_data.values()
        )
        if found_in_pass2:
            print(f"  [Pass 2] {rel_path} -> promoted from Others (codes found)")
        else:
            still_others.append(rel_path)
    others = still_others

    if added:
        print(f"  [Pass 2] {added} new mapping(s) added across all documents.")
    else:
        print(f"  [Pass 2] No additional mappings found.")
...
def second_pass_scan(all_docx: list, folder: str, known_tcodes: set) -> dict:
    """
    Pass 2: Re-scan every document for bare occurrences of already-discovered
    T-codes that were missed because they had no trigger phrase.
    Returns {tcode: [(rel_path, sentence), ...]} of NEW mappings only.
    """
    if not known_tcodes:
        return {}

    # Build one combined regex using strict lookahead/lookbehind
    # to ensure FULL WORD match — not a substring inside a longer code.
    # (?<![A-Z0-9_]) before and (?![A-Z0-9_]) after ensures the code
    # is not preceded or followed by any alphanumeric or underscore character.
    # Sort longest-first so longer codes match before shorter prefixes.
    sorted_codes = sorted(known_tcodes, key=len, reverse=True)
    pattern = re.compile(
        '(?<![A-Z0-9_])(' + '|'.join(re.escape(c) for c in sorted_codes) + ')(?![A-Z0-9_])',
        re.IGNORECASE
    )

    new_hits = defaultdict(list)

    print(f"\n  [Pass 2] Re-scanning {len(all_docx)} document(s) for "
          f"{len(known_tcodes)} known T-code(s)...")
    ...
    for filepath in all_docx:
        ...
        # Collect unique non-empty text blocks (paragraphs + table cells)
        # Use a set to avoid scanning duplicate text blocks (e.g. merged table cells)
        seen_texts = set()
        text_blocks = []
        ...
        # Track (tcode, sentence) pairs already recorded for this doc
        # to avoid duplicates when the same code appears multiple times in same line
        doc_seen = set()
        for text in text_blocks:
            for m in pattern.finditer(text):
                tcode = m.group(1).upper()
                key = (tcode, rel_path, text)
                if key not in doc_seen:
                    doc_seen.add(key)
                    new_hits[tcode].append((rel_path, text))

    return new_hits


def merge_second_pass(tcode_data: dict, new_hits: dict) -> tuple:
    """
    Merge Pass-2 results into tcode_data.
    Only adds (tcode, rel_path, sentence) combos not already present in Pass 1.
    Returns (updated tcode_data, count of new mappings added).
    """
    added = 0
    for tcode, hits in new_hits.items():
        # Build set of already-recorded (rel_path, sentence) pairs for this code
        existing_set = set(tcode_data.get(tcode, []))
        for rel_path, sentence in hits:
            if (rel_path, sentence) not in existing_set:
                tcode_data.setdefault(tcode, []).append((rel_path, sentence))
                existing_set.add((rel_path, sentence))
                added += 1
    return tcode_data, added
