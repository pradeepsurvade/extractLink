...
def scan_folder(folder: str):
    """
    Scan all .docx files in folder AND all subfolders recursively.
    Returns:
    ...
    if not all_docx:
        raise SystemExit(f"No .docx files found in '{folder}' or any of its subfolders.")
    ...
    for filepath in all_docx:
        ...
        if matches:
            ...
            print(f"  ✔ {rel_path} → {len(set(m[0] for m in matches))} t-code(s) found")
        else:
            others.append(rel_path)
