...
def extract_step_sentences(filepath: str, section_text: str, full_text: str) -> list:
    """
    Extract individual process step sentences from the document.
    Returns a list of clean, meaningful step strings.
    Strips region noise so steps compare purely on process content.
    """
    # Try to get steps from the DETAILED PROCESS STEPS section first
    steps_raw = _extract_named_section(filepath, 'DETAILED PROCESS STEPS')
    source = steps_raw or section_text or full_text

    # Split on sentence boundaries and numbered step patterns
    # e.g. "1. Park invoice" "2. Review and approve" "Step 3: Post journal"
    sentences = re.split(r'(?<=[.!?])\s+|(?=\d+[\.\)]\s)|(?=Step\s+\d)', source)

    steps = []
    for s in sentences:
        s = s.strip().strip('•-–—*')
        # Strip leading step numbers: "1.", "1)", "Step 1:", "Step 1 -"
        s = re.sub(r'^(\d+[\.\)]\s*|Step\s*\d+\s*[:\-]?\s*)', '', s, flags=re.IGNORECASE).strip()
        s = strip_region_noise(s)     # remove region noise
        s = re.sub(r'\s+', ' ', s).strip()
        # Keep only meaningful steps (not headers, not too short, not too long)
        if 4 <= len(s.split()) <= 30 and s and not s.isupper():
            steps.append(s)

    return steps[:15]   # cap at 15 steps per doc


def find_similar_steps(doc_steps: list, cluster_steps: list, top_n: int = 4) -> list:
    """
    Find which steps from this document are most similar to the cluster's steps.
    Uses word overlap (Jaccard-like) after synonym expansion and region stripping.
    Returns the top_n matching step sentences from this document.
    """
    if not doc_steps or not cluster_steps:
        return doc_steps[:top_n] if doc_steps else []

    # Build cluster step word sets (expanded synonyms)
    cluster_words = set()
    for step in cluster_steps:
        cluster_words.update(expand_synonyms(clean_text(step)).split())
    # Remove very common words
    cluster_words -= set(STOPWORDS)

    # Score each doc step by overlap with cluster vocabulary
    scored = []
    for step in doc_steps:
        step_words = set(expand_synonyms(clean_text(step)).split()) - set(STOPWORDS)
        if not step_words:
            continue
        overlap = len(step_words & cluster_words) / len(step_words)
        scored.append((overlap, step))

    scored.sort(key=lambda x: -x[0])
    # Return top steps that have some overlap
    return [s for _, s in scored[:top_n] if _ > 0] or doc_steps[:top_n]


def make_reason(doc_kws, cluster_kws, group_name, sections_found,
                used_fallback, focus_mode, doc_steps=None, cluster_steps=None,
    ...
    # ── 3. Matching process steps — the core evidence ────────────────────────
    if doc_steps and cluster_steps:
        similar = find_similar_steps(doc_steps, cluster_steps, top_n=4)
        if similar:
            lines.append("Matching process steps found in this document:")
            for i, step in enumerate(similar, 1):
                display = step[:90] + "..." if len(step) > 90 else step
                lines.append(f"  {i}. {display}")
    elif doc_steps:
        lines.append("Process steps (no other docs in cluster for comparison):")
        for i, step in enumerate(doc_steps[:3], 1):
            display = step[:90] + "..." if len(step) > 90 else step
            lines.append(f"  {i}. {display}")

    # ── 4. Keyword fallback if no step sentences extracted ───────────────────
    if not doc_steps:
