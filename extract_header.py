...
def clean_sop_value(val):
    val = re.sub(r'\s+', ' ', val).strip()
    return re.sub(r'^SOP\s+(?=SOP)', '', val).strip()
    ...
    for row in root.iter(f'{{{W_NS}}}tr'):
        for cell in row.iter(f'{{{W_NS}}}tc'):
            ...
            if ':' in text:
                ...
                label_l = label.lower()
                if 'organization' in label_l:
                    header['Organization'] = value
                elif 'process map' in label_l:
                    header['Process Map No. and Title'] = value
                elif 'sop no' in label_l or ('sop' in label_l and 'title' in label_l):
                    header['SOP No. and Title'] = clean_sop_value(value)
    return header

def scan_docx_folder(folder):
    """Return list of dicts: folder_name, file_name, + all header keys."""
    records = []
    all_header_keys = []  # preserve order of first appearance
    seen_keys = set()
    raw = []
...
DARK_BLUE   = 'FF1F4E79'   # row-1 header background
MED_BLUE    = 'FF2E75B6'   # sub-header (header field names) background
LIGHT_GREY  = 'FFF2F2F2'   # folder/filename cell background
WHITE_FONT  = 'FFFFFFFF'
DARK_FONT   = 'FF1F4E79'
BORDER_DARK = 'FF4472C4'   # medium blue border (folder/file group)
BORDER_GREY = 'FFBFBFBF'   # thin grey border (content cells)
...
def apply_folder_file(cell, value, is_top_row):
    ...
    if is_top_row:
        cell.border = medium_border(BORDER_DARK)
    else:
        cell.border = medium_border_no_top(BORDER_DARK)
...
def build_excel(records, header_keys, output_path):
    ...
    # Column layout:
    # Col A = Folder Name, Col B = File Name, Col C+ = header fields
    all_cols = ['Folder Name', 'File Name'] + header_keys
    ...
    # Merge C1 across all header columns
    if n_header_cols > 1:
        ws.merge_cells(start_row=1, start_column=3,
                       end_row=1,   end_column=2 + n_header_cols)
    ...
    excel_row = 2
    for rec in records:
        # Sub-header row: blank | blank | field-name1 | field-name2 ...
        for ci, key in enumerate(header_keys, start=3):
            apply_sub_header(ws.cell(excel_row, ci), key)

        # Folder/file cells span 2 rows → merge them
        for col_idx in (1, 2):
        ...
        # Value row: blank | blank | value1 | value2 ...
        for ci, key in enumerate(header_keys, start=3):
            apply_data(ws.cell(excel_row, ci), rec.get(key, ''))

        # Bottom part of the merged folder/file cells
        for col_idx in (1, 2):
        ...
        # Merge A and B across the two rows of this record
    ...
    col_letters = 'CDEFGHIJKLMNOPQRSTUVWXYZ'
    for i in range(n_header_cols):
        ws.column_dimensions[col_letters[i]].width = 32
