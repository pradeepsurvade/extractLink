...
def _txt(el):
    # Concatenate all w:t text directly — spaces between words are already
    # embedded within the run text itself (xml:space="preserve").
    # Joining with ' '.join() incorrectly splits words when Word fragments a
    # single word across multiple runs due to spell-check marks, tracked
    # changes, or proofing attributes (e.g. 'I'+'nput' -> 'Input', not 'I nput').
    return ''.join(t.text or '' for t in el.findall('.//{%s}t' % NS['w'])).strip()
...
def extract_from_docx(docx_path, skip_numbering=False):
    ...
    for idx, child in enumerate(children):
        ...
        if tag == 'p':
            ...
            elif content and not heading_seen and idx in pre_table_idx and _is_section_label_para(child):
                # Only use bold pre-table paragraphs as section labels BEFORE the first
                # real heading appears. After a heading is seen, sub-labels like
                # 'Input' / 'Output' within a section are subordinate content, not
                # new section names, and must not override the heading.
                current_section = content
